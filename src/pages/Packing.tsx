...
const EMPTY = { shift: '', no_rak: '', pic: '', barcode: '', description: '', thickness: '', qty: 1 };
...
export default function Packing() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    const { error } = await supabase.from('packing').insert(form);
    setSaving(false);
    if (error) { setAlert({ type: 'error', msg: error.message }); return; }
    setAlert({ type: 'success', msg: 'Data packing berhasil disimpan!' });
    setForm(EMPTY);
    setAutoFilled(false);
  }
    ...
    <div className="space-y-4 animate-fade-in max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="form-section">
        <div className="flex items-center justify-between pb-2 border-b border-white/30">
          <h3 className="text-slate-800 font-bold text-base">Input Data Packing</h3>
        ...
        <button type="submit" disabled={saving} className="glass-btn-primary flex items-center gap-2 w-full justify-center py-3">
          ...
          {saving ? 'Menyimpan...' : 'Simpan Data Packing'}
        </button>
      </form>
    </div>
  );
