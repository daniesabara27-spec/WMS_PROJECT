...
const JENIS = ['Dari KCC', 'Dari Wanxinda', 'Transfer Internal'];
const PENEMPATAN = ['Gudang A', 'Gudang B', 'Gudang C', 'Area Transit'];

const EMPTY = {
  jenis_penerimaan: '', no_surat_jalan: '', penempatan_gudang: '',
  shift: '', no_rak: '', pic: '', barcode: '', description: '', thickness: '', qty: 1,
};
...
export default function Inbound() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    const { error } = await supabase.from('inbound').insert(form);
    setSaving(false);
    if (error) { setAlert({ type: 'error', msg: error.message }); return; }
    setAlert({ type: 'success', msg: 'Data inbound berhasil disimpan!' });
    setForm(EMPTY);
    setAutoFilled(false);
  }
    ...
    <div className="space-y-4 animate-fade-in max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="form-section">
        <div className="flex items-center justify-between pb-2 border-b border-white/30">
          <h3 className="text-slate-800 font-bold text-base">Input Data Inbound</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField label="Jenis Penerimaan" required>
            <select className="glass-select" value={form.jenis_penerimaan} onChange={f('jenis_penerimaan')} required>
          ...
          <FormField label="Penempatan Gudang">
            <select className="glass-select" value={form.penempatan_gudang} onChange={f('penempatan_gudang')}>
              <option value="">-- Pilih Gudang --</option>
              {PENEMPATAN.map((p) => <option key={p}>{p}</option>)}
            </select>
          </FormField>
          ...
          <FormField label="No. Rak">
            <input className="glass-input" value={form.no_rak} onChange={f('no_rak')} placeholder="A-01" />
          </FormField>
        ...
        <button type="submit" disabled={saving} className="glass-btn-primary flex items-center gap-2 w-full justify-center py-3">
          ...
          {saving ? 'Menyimpan...' : 'Simpan Data Inbound'}
        </button>
      </form>
    </div>
  );
